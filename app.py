...
st.write('This ML-based app is developed for educational purposes. The objective of the app is detecting phishing websites only using content data, not the URL.'
...
with st.expander("PROJECT DETAILS"):
    ...
    st.write('I used _supervised learning_ to classify phishing and legitimate websites. '
             'I benefitted from a content-based approach and focused on the HTML of the websites. '
    ...
    number = st.slider("Select row number to display", 0, 100, key="slider1")
    ...
    st.write('I used only content-based features. I didn\'t use URL-based features like length of URL etc.'
             ' Most of the features were extracted using the find_all() method of the BeautifulSoup module after parsing HTML.')

    st.subheader('Results')
    st.write('I used 7 different ML classifiers from scikit-learn and tested them implementing k-fold cross-validation.'
             ' Firstly, I obtained their confusion matrices, then calculated their accuracy, precision, and recall scores.'
...
with st.expander('EXAMPLE PHISHING URLs:'):
    ...
    st.caption('REMEMBER, PHISHING WEB PAGES HAVE A SHORT LIFECYCLE! SO, THE EXAMPLES SHOULD BE UPDATED!')
...
if st.button('Check!', key="check_button"):
    try:
        ...
        if response.status_code != 200:
            st.warning("HTTP connection was not successful for the URL: {}".format(url))
        else:
        ...
        st.error("An error occurred while processing the URL: {}".format(str(e)))
